#!/bin/bash
# One GPU chip type; the array task id picks one dtype:sweep:exponent point from the args.
# Args: <label> <config>... Submitted by `python -m bench functions submit`.
set -euo pipefail
cd "$SLURM_SUBMIT_DIR"
...
uv run --with "jax[cuda12]" python -m bench functions run \
  --sweep "$sweep" --dtype "$dtype" --exponent "$exponent" --label "$label" \
