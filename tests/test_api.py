...
def instantiate(cls, any_d=8, **kwargs):
    # every field without a default has to be supplied: `d` takes any dimension (a fixed-dimension
    # class hardcodes its own and has no such field), a Literal takes its first published option
...
def sample(f, n=7, seed=0):
    return jr.uniform(jr.key(seed), (n, dim(f)))
...
def test_domain_bounds_broadcast_and_are_ordered(cls):
    ...
    shapes = (jnp.shape(lo), jnp.shape(hi), (dim(f),))
    assert jnp.broadcast_shapes(*shapes) == (dim(f),)
...
def test_is_jittable_and_differentiable(cls):
    ...
    u = sample(f)
    # eqx.filter_jit, not jax.jit: a module carrying array tables is not hashable as a static arg
...
def test_normalized_branches_the_same_when_the_module_is_a_jitted_argument(cls):
    # no field is static: filter_jit holds every non-array leaf, so `if self.normalized`
    # and the other python-level branches never see a tracer
